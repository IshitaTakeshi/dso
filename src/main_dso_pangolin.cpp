...
int main( int argc, char** argv )
{
    ...
    for(int i=1; i<argc; i++) {
        parseArgument(argv[i],
                      ...
                      start, end, reverse, playbackSpeed,
                      useSampleOutput, preload);
    }
    ...
    std::thread runthread([&]() {
                ...
                && linc*i < linc*lend; i+=linc)
        {
            ...
            else
            {
                ...
                timesToPlayAt.push_back(timesToPlayAt.back() +  fabs(tsThis
        ...
        double MilliSecondsTakenSingle = 1000.0f*(ended-started)/(float)(
                                             CLOCKS_PER_SEC);
        double MilliSecondsTakenMT = sInitializerOffset + ((tv_end.tv_sec
        ...
        if(setting_logStuff)
        {
            ...
            tmlog << 1000.0f*(ended-started)/(float)(CLOCKS_PER_SEC*reader->getNumImages())
                  << " "
                  << ((tv_end.tv_sec-tv_start.tv_sec)*1000.0f + (tv_end.tv_usec
                          -tv_start.tv_usec)/1000.0f) / (float)reader->getNumImages() << "\n";
            tmlog.flush();
            tmlog.close();
        }
