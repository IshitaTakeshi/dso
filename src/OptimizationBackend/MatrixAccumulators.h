...
private:
    ...
    void shiftUp(bool force)
    {
        ...
        {
            for(int i=0; i<105; i++)
                _mm_store_ps(SSEData1m+4*i, _mm_add_ps(_mm_load_ps(SSEData1k+4*i),
    ...
    void shiftUp(bool force)
    {
        if(numIn1 > 1000 || force)
        {
            ...
            for(int i=0; i<32; i+=4)
                _mm_store_ps(TopRight_Data1k+i, _mm_add_ps(_mm_load_ps(TopRight_Data+i),
                             _mm_load_ps(TopRight_Data1k+i)));
            for(int i=0; i<8; i+=4)
                _mm_store_ps(BotRight_Data1k+i, _mm_add_ps(_mm_load_ps(BotRight_Data+i),
        ...
        if(numIn1k > 1000 || force)
        {
            ...
            for(int i=0; i<32; i+=4)
                _mm_store_ps(TopRight_Data1m+i, _mm_add_ps(_mm_load_ps(TopRight_Data1k+i),
                             _mm_load_ps(TopRight_Data1m+i)));
            for(int i=0; i<8; i+=4)
                _mm_store_ps(BotRight_Data1m+i, _mm_add_ps(_mm_load_ps(BotRight_Data1k+i),
