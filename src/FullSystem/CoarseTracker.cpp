                              ...
                              const SE3 &refToNew, AffLight aff_g2l)
{
    ...
    for(int i=0; i<n; i+=4)
    {
        ...
        acc.updateSSE_eighted(
            _mm_mul_ps(id,dx),
            _mm_mul_ps(id,dy),
            _mm_sub_ps(zero, _mm_mul_ps(id,_mm_add_ps(_mm_mul_ps(u,dx), _mm_mul_ps(v,
                                        dy)))),
    ...
    for(int lvl=coarsestLvl; lvl>=0; lvl--)
    {
        ...
        if(debugPrint)
        {
            Vec2f relAff = AffLight::fromToVecExposure(lastRef->ab_exposure,
                           newFrame->ab_exposure, lastRef_aff_g2l, aff_g2l_current).cast<float>();
        ...
        for(int iteration=0; iteration < maxIterations[lvl]; iteration++)
        {
            ...
            if(lambda < lambdaExtrapolationLimit) extrapFac = sqrt(sqrt(
                            lambdaExtrapolationLimit / lambda));
            inc *= extrapFac;
