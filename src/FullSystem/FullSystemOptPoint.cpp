    ...
    ImmaturePointTemporaryResidual* residuals)
{
    ...
    {
        lastEnergy += point->linearizeResidual(&camera_parameters, 1000, residuals+i, lastHdd,
    ...
    for(int iteration=0; iteration<setting_GNItsOnPointActivation; iteration++)
    {
        ...
        float newEnergy=0;
        for(int i=0; i<nres; i++)
            newEnergy += point->linearizeResidual(&camera_parameters, 1, residuals+i,newHdd, newbd,
                                                  newIdepth);
    ...
    for(int i=0; i<nres; i++)
        if(residuals[i].state_state == ResState::IN)
        {
            PointFrameResidual* r = new PointFrameResidual(
                p, p->host, residuals[i].target, ResState::IN);
            ...
            else if(r->target == (frameHessians.size()<2 ? 0 :
