...
void KeyFrameDisplay::setFromKF(FrameHessian* fh, CalibHessian* HCalib)
{
    ...
    if(numSparseBufferSize < npoints)
    {
        ...
        originalInputSparse = new
        InputPointSparse<MAX_RES_PER_POINT>[numSparseBufferSize];
    }
                                ...
                                int mode, float minBS, int sparsity)
{
    ...
    for(int i=0; i<numSparsePoints; i++)
    {
        ...
        if(my_displayMode==1 && originalInputSparse[i].status != 1
                && originalInputSparse[i].status!= 2) continue;
        ...
        for(int pnt=0; pnt<patternNum; pnt++)
        {
            ...
            tmpVertexBuffer[vertexBufferNumPoints][0] = ((originalInputSparse[i].u+dx)*fxi
                    + cxi) * depth;
            tmpVertexBuffer[vertexBufferNumPoints][1] = ((originalInputSparse[i].v+dy)*fyi
                    + cyi) * depth;
    ...
    if(numGLBufferGoodPoints > numGLBufferPoints)
    {
        ...
        vertexBuffer.Reinitialise(pangolin::GlArrayBuffer, numGLBufferPoints, GL_FLOAT,
                                  3, GL_DYNAMIC_DRAW );
