...
EIGEN_ALWAYS_INLINE float getInterpolatedElement31(const Eigen::Vector3f* const
...
EIGEN_ALWAYS_INLINE float getInterpolatedElement11Cub(const float* const p,
        const float x)	// for x=0, this returns p[1].
{
    return p[1] + 0.5f * x*(p[2] - p[0] + x*(2.0f*p[0] - 5.0f*p[1] + 4.0f*p[2] -
                            p[3] + x*(3.0f*(p[1] - p[2]) + p[3] - p[0])));
}
...
EIGEN_ALWAYS_INLINE Eigen::Vector2f getInterpolatedElement32Cub(
    const Eigen::Vector3f* const p, const float x)	// for x=0, this returns p[1].
{
...
inline Vec3b makeJet3B(float id)
{
    ...
    if(icP == 4) return Vec3b(255*(0.5-0.5*ifP), 					255,
                                  255*(0.5+0.5*ifP));
